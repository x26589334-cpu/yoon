# 2026-09-03 지역 자판기 후기 3편 목록 반영 + 페이지 재정리 + sitemap 갱신
...
$posts=@(
 @{ slug='2026-09-03-jeonju-vending'; img='post-0903-jeonju.jpg'; cat='전주';
    title='전주 오피스텔 무인자판기 설치 — 원룸촌 로비 24시간 무인 판매';
    alt='전주 오피스텔 무인자판기 설치 후기 - H포스';
    sum='원룸촌 건물 로비에 음료·스낵 자판기 설치. 심야 편의점 수요 대체, 카드·간편결제, 원격 재고 관리, 설치비 무료.' },
 @{ slug='2026-09-03-changwon-vending'; img='post-0903-changwon.jpg'; cat='창원';
    title='창원 스터디카페 무인자판기 설치 — 독서실 라운지 24시간 무인 판매';
    alt='창원 스터디카페 무인자판기 설치 후기 - H포스';
    sum='독서실 라운지에 스낵·음료 자판기 설치. 오래 머무는 공간이라 회전이 좋고, 무인 매장과 궁합이 맞습니다. 설치비 무료.' },
 @{ slug='2026-09-03-seongnam-vending'; img='post-0903-seongnam.jpg'; cat='성남';
    title='성남 지식산업센터 무인자판기 설치 — 판교·야탑 오피스 직원용 24시간';
    alt='성남 지식산업센터 무인자판기 설치 후기 - H포스';
    sum='판교·야탑 오피스 휴게실에 냉장 음료 자판기 설치. 상주 인원 고정이라 매일 반복 구매, 24시간 무인 판매. 설치비 무료.' }
)
...
# 새 글 날짜가 목록 맨 위 글보다 과거면, 날짜순이 맞도록 그 아래에 끼워 넣습니다.
# ($insertAt = 앞에서 몇 개를 건너뛰고 넣을지)
$insertAt=1
if($insertAt -gt 0){
  $all=@($existing[0..($insertAt-1)])+@($newCards)+@($existing[$insertAt..($existing.Count-1)])
} else {
  $all=@($newCards)+@($existing)
}
$per=10; $total=$all.Count; $tp=[math]::Ceiling($total/$per)

# 3) 목록 페이지 재작성 (페이지 수가 줄면 남는 파일 삭제)
for($i=1;$i -le $tp;$i++){
...
# 4) sitemap 갱신 — 새 글 추가(후기는 0.7) + 목록 페이지 lastmod 갱신
