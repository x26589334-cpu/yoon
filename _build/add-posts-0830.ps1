# 2026-08-30 후기 3편(창원 포스기·전주 카드단말기·구미 자판기)을 목록 맨 앞에 추가 + 사이트맵. 라이브 형식 보존, 카운트 정규식.
$ErrorActionPreference='Stop'
$repo="$env:USERPROFILE\Desktop\yoon"
$enc=New-Object System.Text.UTF8Encoding($false)

$new=@(
 @{f='blog/2026-08-30-gumi-vending.html'; img='photos/vend-yongin.jpg'; alt='구미 공장 휴게실 직원용 무인자판기 여러 대 설치 - H포스'; label='구미 자판기'; h='구미 무인자판기 설치 완료 — 공장 휴게실 직원용 자판기'; p='경북 구미 공장 휴게실에 음료·스넥·간식 무인자판기 여러 대를 설치했습니다. 셀프 결제·앱 원격관리로 24시간 무인 운영, 야간·교대 근무 많은 공장이라 고정 수요가 확실한 자리.'}
 @{f='blog/2026-08-30-jeonju-card.html'; img='photos/pos-install-45.jpg'; alt='전주 효자동 카페 카드단말기 인터넷선 연결 설치 - H포스'; label='전주 카드단말기'; h='전주 카드단말기 설치 완료 — 효자동 카페 결제 세팅'; p='전북 전주 효자동 카페에 인터넷선(랜선) 방식 유선 카드단말기를 새로 설치했습니다. 승인 속도 개선, 기존 가맹 그대로 번호 유지, 간편결제·QR까지 세팅. 교체도 설치비 무료.'}
 @{f='blog/2026-08-30-changwon-pos.html'; img='photos/pos-install-40.jpg'; alt='창원 상남동 음식점 포스기·카드단말기 설치 - H포스'; label='창원 포스기'; h='창원 포스기·카드단말기 설치 완료 — 상남동 음식점 결제 구축'; p='경남 창원 상남동 음식점에 포스기와 카드단말기를 한 번에 설치했습니다. 메뉴·테이블을 포스에 등록하고 카드·간편결제·QR까지 연동해 계산 실수를 줄였습니다.'}
)
$newCards=@()
foreach($n in $new){
  $newCards += @"
<a class="post-card" href="$($n.f)">
      ...
      <div class="pc-body">
        <div class="pc-date">2026.08.30 · $($n.label)</div>
        <h2>$($n.h)</h2>
        <p>$($n.p)</p>
      </div>
    </a>
...
foreach($n in $new){
  if($sc -notmatch [regex]::Escape($n.f)){
    $line='  <url><loc>https://hpos.co.kr/'+$n.f+'</loc><lastmod>2026-08-30</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>'
